...
const ServiceDetails: React.FC<ServiceDetailsProps> = ({ isOpen, onClose, service }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-1 sm:p-4">
      {/* Backdrop */}
      <div
        className="absolute inset-0 backdrop-blur-md bg-black/50 transition-all duration-200"
        onClick={onClose}
      />
      ...
      <motion.div
        ...
        className="relative w-full max-w-2xl sm:max-w-3xl md:max-w-4xl max-h-[90vh] sm:max-h-[95vh] overflow-y-auto bg-white rounded-2xl shadow-2xl"
      ...
      >
        ...
        <div className="relative h-48 xs:h-56 sm:h-64 md:h-80 rounded-t-2xl overflow-hidden">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-8">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-serif font-bold text-white mb-2 line-clamp-2">
              {t(`services.${service.title}.title`)}
            </h2>
            <p className="text-white/90 text-base sm:text-lg line-clamp-2">
          ...
          <button
            onClick={onClose}
            className="absolute top-3 right-3 sm:top-4 sm:right-4 z-20 w-12 h-12 sm:w-10 sm:h-10 rounded-full flex items-center justify-center bg-white/90 hover:bg-white text-darkgreen hover:text-gold transition-all duration-300 hover:scale-110 shadow-lg border-2 border-white/20"
            aria-label="Fermer"
          >
            <svg
                ...
                {service.details.map((detail, index) => (
                  <li key={index} className="flex items-start space-x-3">
                    ...
                    <span className="text-gray-700">
                      {t(`services.${service.title}.details.${index}`)}
                    </span>
                  </li>
                ))}
              </ul>
              ...
              <ul className="grid md:grid-cols-2 gap-4">
                {service.advantages.map((advantage, index) => (
                  <li key={index} className="flex items-start space-x-3">
                    ...
                    <span className="text-gray-700">
                      {t(`services.${service.title}.advantages.${index}`)}
                    </span>
                  </li>
                ))}
              </ul>
